      ...
      <section style={{ padding: "80px 0", background: "#FFFFFF" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 24px" }}>
          <AnimatedContent style={{ textAlign: "center", marginBottom: 36 }}>
            <span className="eyebrow" style={{ color: "#B30F14" }}>Item Pricing</span>
            ...
            <p style={{ color: "#6B6B6B", fontFamily: "Kodchasan, sans-serif", maxWidth: "52ch", margin: "0 auto" }}>
              Prices marked with a <strong style={{ color: "#B30F14" }}>+</strong> are starting prices — the final price depends on size, fabric and condition, and is always confirmed with you first.
            </p>
          </AnimatedContent>
              ...
              {CATALOG.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    border: activeTab === t.id ? "1.5px solid #ED1D24" : "1.5px solid rgba(20,20,20,0.12)",
                    background: activeTab === t.id ? "#ED1D24" : "#ffffff",
              ...
              <div style={{ display: "grid", gridTemplateColumns: tab.sections.length > 1 ? "repeat(2,1fr)" : "minmax(0,560px)", gap: 20, justifyContent: "center" }} className="catalog-grid">
                {tab.sections.map((section) => (
                  <div key={section.title} style={{ background: "#ffffff", borderRadius: 20, padding: "26px 28px", border: "1px solid rgba(20,20,20,0.06)", boxShadow: "0 1px 2px rgba(0,0,0,0.04), 0 4px 16px rgba(0,0,0,0.03)" }}>
                    <h3 style={{ fontFamily: "Poppins, sans-serif", fontWeight: 600, fontSize: "0.8rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "#B30F14", marginBottom: 16 }}>
                      {section.title}
                    </h3>
                      ...
                      {section.items.map((item, i) => (
                        <li key={item.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 16, padding: "10px 0", borderBottom: i < section.items.length - 1 ? "1px solid rgba(20,20,20,0.06)" : "none" }}>
                          ...
                          <span style={{ fontFamily: "Poppins, sans-serif", fontWeight: 600, fontSize: "0.9rem", color: "#161616", whiteSpace: "nowrap" }}>
                            ${item.price.toFixed(2)}{item.from && <span style={{ color: "#B30F14" }}> +</span>}
                          </span>
                        </li>
                      ))}
                    </ul>
      ...
      <section style={{ padding: "80px 0", background: "#F2F2F2" }}>
        <div style={{ maxWidth: 980, margin: "0 auto", padding: "0 24px" }}>
          <AnimatedContent style={{ textAlign: "center", marginBottom: 48 }}>
            <span className="eyebrow" style={{ color: "#B30F14" }}>Choose Your Plan</span>
                ...
                <ul style={{ listStyle: "none", marginBottom: 28, display: "flex", flexDirection: "column", gap: 10, flexGrow: 1 }}>
                  {planPPP.features.map((f) => (
                    <li key={f} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                      <CheckCircle size={15} color="#ED1D24" style={{ flexShrink: 0, marginTop: 2 }} />
      ...
      <section style={{ padding: "80px 0", background: "#FFFFFF" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <AnimatedContent style={{ marginBottom: 40 }}>
            <span className="eyebrow" style={{ color: "#B30F14" }}>At A Glance</span>
            <h2 style={{ fontFamily: "Poppins, sans-serif", fontWeight: 600, fontSize: "clamp(1.75rem,3.5vw,2.5rem)", letterSpacing: "-0.022em", color: "#161616", marginBottom: 8 }}>
            ...
            <p style={{ color: "#6B6B6B", maxWidth: "50ch", fontFamily: "Kodchasan, sans-serif" }}>
              See the full breakdown on the <a href="/services" style={{ color: "#B30F14", textDecoration: "underline" }}>Services</a> page.
            </p>
          </AnimatedContent>
            ...
            {payAsYouGo.map((s, i) => (
              <AnimatedContent key={s.title} delay={i * 0.06}>
                <div style={{ background: pastelColors[i], borderRadius: 20, padding: "28px", position: "relative", height: "100%" }}>
                  {s.badge && (
                    <span style={{ position: "absolute", top: 20, right: 20, background: "#ED1D24", color: "#ffffff", fontSize: "0.65rem", fontWeight: 700, padding: "3px 9px", borderRadius: 999, fontFamily: "Kodchasan, sans-serif" }}>
                      {s.badge}
                    </span>
                  )}
          ...
          <AnimatedContent>
            <p style={{ color: "#8C8C8C", fontSize: "0.875rem", marginTop: 24, textAlign: "center", fontFamily: "Kodchasan, sans-serif" }}>
              All prices shown are before HST ({HST_LABEL}). ${MINIMUM_ORDER.standardCad} minimum order value before HST (${MINIMUM_ORDER.detailingCad} for Car &amp; Sofa Detailing). Free pickup &amp; delivery included on every order. We weigh at pickup and confirm your price before washing.
            </p>
          </AnimatedContent>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "0 24px" }}>
          <AnimatedContent>
            <div style={{ background: "#F2F2F2", borderRadius: 24, padding: "40px", textAlign: "center" }}>
              <span className="eyebrow" style={{ color: "#B30F14" }}>{DRY_CLEAN_COMBO.title}</span>
