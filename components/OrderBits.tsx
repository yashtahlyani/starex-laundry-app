...
export const STATUS_META: Record<string, { label: string; bg: string; fg: string; dot: string }> = {
  ...
  washing:          { label: "Being cleaned",     bg: "#F5F1EE", fg: "#047857", dot: "#A63446" },
      ...
      {ORDER_STAGES.map((s, i) => {
        ...
        return (
          <div key={s.id} style={{ display: "flex", alignItems: "center", flex: i < ORDER_STAGES.length - 1 ? 1 : undefined }}>
            <motion.div
              animate={{ background: done ? "#A63446" : "#E4E4E7", scale: active ? 1.15 : 1 }}
            ...
            {i < ORDER_STAGES.length - 1 && (
              <div style={{ flex: 1, height: 2, background: done && i < current ? "#A63446" : "#E4E4E7", margin: "0 2px", transition: "background 0.3s" }} />
            )}
          </div>
        );
      })}
